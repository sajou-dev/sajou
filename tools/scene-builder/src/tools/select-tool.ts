...
import { screenToScene, getCanvasContainer, getLayers, getZoom } from "../canvas/canvas.js";
import {
  getEditorState,
  setSelection,
  showPanel,
...
// ---------------------------------------------------------------------------
// Center guide lines — shown during entity drag
// ---------------------------------------------------------------------------
...
/**
 * Apply center-snap: if the entity center is within threshold of the scene
 * center axis, snap it to that axis. Works independently on X and Y.
 */
function snapToCenter(x: number, y: number): { x: number; y: number } {
  const { dimensions } = getSceneState();
  const cx = dimensions.width / 2;
  const cy = dimensions.height / 2;
